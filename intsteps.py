...
class IntegralPrinter(object):
    ...
    def print_Parts(self, rule):
        with self.new_step():
            ...
            self.append(self.format_math_display(r"\int \operatorname{u} \operatorname{dv} = \operatorname{u}\operatorname{v} - \int \operatorname{v} \operatorname{du}"))
...
class HTMLPrinter(IntegralPrinter, stepprinter.HTMLPrinter):
    ...
    def format_math_constant(self, math):
        return ' $'+(sympy.latex(math) + r'+ \mathrm{constant}')+'$ '
